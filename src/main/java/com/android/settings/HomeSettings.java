...
public class HomeSettings extends SettingsPreferenceFragment implements Indexable {
    ...
    private void buildHomeActivitiesList() {
        ...
        int prefIndex = 0;
        boolean mustSupportManagedProfile = hasManagedProfile() || getActivity().getIntent().getBooleanExtra("support_managed_profiles", false);
        for (int i = 0; i < homeActivities.size(); i++) {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (args == null || !args.getBoolean("show", false)) {
            z = false;
        } else {
            z = true;
        }
